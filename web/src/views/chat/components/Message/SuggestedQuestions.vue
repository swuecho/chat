  ...
  >
    <div class="flex items-center justify-between mb-2">
      <div class="flex items-center">
        ...
        <div v-if="generating" class="ml-2">
          <div class="animate-spin rounded-full h-3 w-3 border-b-2 border-green-500" />
        </div>
      </div>

      <!-- Batch indicator and navigation -->
      <div
        v-if="batches && batches.length > 1"
        class="flex items-center space-x-1 text-xs text-gray-500 dark:text-gray-400"
      >
        <span>{{ currentBatchNumber }}/{{ totalBatches }}</span>
        <button
          :disabled="!hasPreviousBatch" class="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <button
          :disabled="!hasNextBatch" class="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
    ...
    <div v-if="!loading && questions.length > 0" class="space-y-2">
      ...
      >
        ...
        <div v-if="generating" class="animate-spin rounded-full h-4 w-4 border-b-2 border-current" />
