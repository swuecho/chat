...
-- Create new bot answer history entry
CREATE OR REPLACE FUNCTION create_bot_answer_history(
    bot_uuid VARCHAR(255),
    user_id INTEGER,
    prompt TEXT,
    answer TEXT,
    model VARCHAR(255),
    tokens_used INTEGER
) RETURNS bot_answer_history AS $$
...
-- Get bot answer history by ID
CREATE OR REPLACE FUNCTION get_bot_answer_history_by_id(
    id INTEGER
) RETURNS bot_answer_history AS $$
SELECT * FROM bot_answer_history WHERE id = $1;
$$ LANGUAGE sql;

-- Get bot answer history by bot UUID
CREATE OR REPLACE FUNCTION get_bot_answer_history_by_bot_uuid(
    bot_uuid VARCHAR(255),
...
SELECT * FROM bot_answer_history 
WHERE bot_uuid = $1
ORDER BY created_at DESC
LIMIT $2 OFFSET $3;
$$ LANGUAGE sql;

-- Get bot answer history by user ID
CREATE OR REPLACE FUNCTION get_bot_answer_history_by_user_id(
    user_id INTEGER,
    limit_count INTEGER DEFAULT 100,
    offset_count INTEGER DEFAULT 0
) RETURNS TABLE (
...
-- Update bot answer history
CREATE OR REPLACE FUNCTION update_bot_answer_history(
    id INTEGER,
    answer TEXT,
    tokens_used INTEGER
) RETURNS bot_answer_history AS $$
...
-- Delete bot answer history by ID
CREATE OR REPLACE FUNCTION delete_bot_answer_history(
    id INTEGER
) RETURNS VOID AS $$
DELETE FROM bot_answer_history WHERE id = $1;
$$ LANGUAGE sql;

-- Get bot answer history count by bot UUID
CREATE OR REPLACE FUNCTION get_bot_answer_history_count_by_bot_uuid(
    bot_uuid VARCHAR(255)
) RETURNS BIGINT AS $$
SELECT COUNT(*) FROM bot_answer_history WHERE bot_uuid = $1;
$$ LANGUAGE sql;

-- Get bot answer history count by user ID
CREATE OR REPLACE FUNCTION get_bot_answer_history_count_by_user_id(
    user_id INTEGER
) RETURNS BIGINT AS $$
SELECT COUNT(*) FROM bot_answer_history WHERE user_id = $1;
$$ LANGUAGE sql;

-- Get latest bot answer history by bot UUID
CREATE OR REPLACE FUNCTION get_latest_bot_answer_history_by_bot_uuid(
    bot_uuid VARCHAR(255),
