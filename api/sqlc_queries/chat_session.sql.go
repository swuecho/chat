...
VALUES ($1, $2, $3, $4)
RETURNING id, user_id, uuid, topic, created_at, updated_at, active, max_length, temperature
`
...
SELECT id, user_id, uuid, topic, created_at, updated_at, active, max_length, temperature FROM chat_session 
where active = true
ORDER BY id
`
...
SELECT id, user_id, uuid, topic, created_at, updated_at, active, max_length, temperature FROM chat_session WHERE id = $1
`
...
SELECT cs.id, cs.user_id, cs.uuid, cs.topic, cs.created_at, cs.updated_at, cs.active, cs.max_length, cs.temperature
