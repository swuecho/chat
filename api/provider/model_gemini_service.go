...
func (m *GeminiChatModel) Stream(ctx context.Context, w http.ResponseWriter, chatSession sqlc_queries.ChatSession, messages []models.Message, chatUuid string, regenerate bool, stream bool) (*models.LLMAnswer, error) {
	...
	if err != nil {
		return nil, dto.ErrInternalUnexpected.WithMessage("Failed to generate regular response").WithDebugInfo(err.Error())
	}
	if llmAnswer == nil {
		return nil, dto.ErrInternalUnexpected.WithMessage("Empty response from Gemini")
	}

	llmAnswer.AnswerId = answerID
	response := buildStreamResponse(answerID, llmAnswer.Answer)
...
func (m *GeminiChatModel) handleStreamResponse(ctx context.Context, w http.ResponseWriter, req *http.Request, answerID string) (*models.LLMAnswer, error) {
	...
	if err != nil {
		return nil, dto.ErrInternalUnexpected.WithMessage("Failed to send Gemini API request").WithDebugInfo(err.Error())
	}
	...
	for count := 0; count < 10000; count++ {
		...
		if err != nil {
			...
			return nil, dto.ErrInternalUnexpected.WithMessage("Error reading stream").WithDebugInfo(err.Error())
		}
