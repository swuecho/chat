...
func TestProviderImplementationsUseCancellationAwareChunkDelivery(t *testing.T) {
	err := filepath.WalkDir(".", func(path string, entry os.DirEntry, err error) error {
		if err != nil {
			return err
		}
		...
		if strings.Contains(string(content), "ch <- StreamChunk") {
			t.Errorf("%s sends a chunk without observing cancellation; use emitChunk", path)
		}
		return nil
	})
	if err != nil {
